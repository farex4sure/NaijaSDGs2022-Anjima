...
if(isset($_POST['submit'])){
    ...
    if($e_image != ""){
    $update = "UPDATE vendors SET pic = '$e_image' WHERE phone='".$_SESSION['loggedin_vendor']."'";
    $updated = mysqli_query($conn,$update);

    if($e_name != ""){
    $update = "UPDATE vendors SET fullname = '$e_name' WHERE phone='".$_SESSION['loggedin_vendor']."'";
    $updated = mysqli_query($conn,$update);
    }
    if($e_email != ""){
    $update = "UPDATE vendors SET email = '$e_email' WHERE phone='".$_SESSION['loggedin_vendor']."'";
    $updated = mysqli_query($conn,$update);
    }
    $err= '
    <div role="alert">
        ...
        <header class='flex justify-center p-3 shadow mb-2 md:px-8 bg-gray-100 bg-opacity-25'>
            <div class='flex items-center w-full max-w-6xl'>
                <div class='flex items-center gap-2 mr-auto'>
                    <a href="dashboard.php" class="text-teal-600 text-xl md:text-2xl lg:text-3xl font-bold"><span><i class="fa fa-arrow-left"></i></span></a>
                ...
                <div class='flex flex-col w-full gap-3 px-3 py-4 rounded-lg'>
                    <div class="flex flex-col items-center bg-white bg-opacity-50 py-5 px-3 rounded-xl">
                        <div class="relative flex justify-center items-center w-20 h-20 border border-teal-600 text-teal-50 rounded-full">
                            <div class="flex justify-center items-center rounded-full w-full h-full text-2xl md:text-3xl overflow-hidden">
                                <img class='w-full h-full object-cover' src="vendor/<?php echo $pic ?>" alt="user_photo">
                            </div>
                        </div>
                    ...
                    <div class='flex flex-col divide-y border-t border-b rounded-md justify-start bg-white bg-opacity-50 py-6 px-3 md:px-6 lg:px-8'>
                        <form action="edit_profile.php" method="post">
                            ...
                            <div class="relative z-0 mb-6 w-full group">
                                <input type="email" name="email" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-teal-600 peer" placeholder=" " value='<?php echo $email ?>'  required="">
                            ...
                            <div class="grid md:grid-cols-2 md:gap-6">
                                <div class="relative z-0 mb-6 w-full group">
                                    <input type="tel" name="phone" disabled class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-teal-500 focus:outline-none focus:ring-0 focus:border-teal-600 peer" value="<?php echo $_SESSION['loggedin_vendor'] ?>" placeholder=" " required="">
